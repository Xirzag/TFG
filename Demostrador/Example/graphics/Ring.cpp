...
Ring::Ring(float innerRadius, float outerRadius, int points) : innerRadius(innerRadius), outerRadius(outerRadius), points(points)
{
	n_vertices = points * 2;
	n_triangles = (points) * 2;
	vertices = new Position[points * 2];
	normal = new Position[points * 2];
	textCoord = new Position2D[points * 2];
	index = new int[n_triangles * 3];
	...
	int addedCircle = 0;
	for (int i = 0; i < n_triangles*3; i+=6) {
	...
	index[n_triangles * 3 - 1] = 1;
	index[n_triangles * 3 - 2] = 0;
	index[n_triangles * 3 - 4] = 0;
